...
We at the Data Lab really like using notebook documents like [Rmarkdown](https://rmarkdown.rstudio.com) (this document) and [Jupyter](https://jupyter.org) for development and data exploration.
Notebooks allow mixing of text, code, and results in a single document that is easy to work with interactively.
This allows much more freedom to include a lot of explanatory text, much like code comments, but with simpler syntax and the ability to include formatting and links more easily through the use of [markdown-formatted text](https://rmarkdown.rstudio.com/lesson-8.html) (or using the new visual editor in RStudio).

When you are ready to share, notebooks like this can easily be converted into a document such as `.html` (web page) or `.docx` (MS Word document) for sharing with others.

**A note on formatting:**
We like to keep our documents formatted with the philosophy of one sentence per line.
This interacts well with git and GitHub, which record edits based on the line that changed.
...
We will also take advantage of the fact that the file.path() function (and `here::here()`), allows us to specify paths as a series of comma-separated partial paths, i.e., individual directories and files.
If we knew everyone were using a UNIX machine, we could use a path like `analysis/gene_counts`, but this is a bit less portable to other systems.
(Thankfully, paths like this *do* work on Windows now!) Using the separated list of elements also makes it easier to join partial paths together.
...
We'll give it some options to allow filtering to a minimum number of samples mutated, and highlighting of particular genes of interest.
...
To make this a bit more structured we will use a format called roxygen for these comments, which would be useful for automatically generating documentation if we were creating an R package.
We aren't doing that here, but it is still a nice enough convention, and RStudio has an option in the `Code` menu called `Insert Roxygen Skeleton` that can save us a bunch of typing and create the outline of what we need in a single step.
...
Your preferred method is up to you: here we are using names for chunks with defined purpose, but not for the more exploratory, iterative chunks in this section and the next.
...
.
In most cases, this will be just fine, but if the output here were likely to be feeding into another analysis, I would probably want to be more careful about defining all the outputs at the top of the notebook.

```{r save_plots}
ggsave(file.path(plots_dir, "LGAT_mutation-counts.png"), mb_plot)
...
The most common such function is `sessionInfo()`, which is part of base R and is what I am using here, but a nice alternative is the function `sessioninfo::session_info()` (part of the `sessioninfo` package\`), which provides a somewhat more neatly-formatted report with a bit more information.
