...
However, we don't always load all of the packages we plan to use, as loading a package does take some time.
...
This can be more efficient if you are only using a single function from a package, and provides extra context about which package provided a particular function, which can be easy to lose track of if many packages are loaded.


The `::` notation also can avoid errors when more than one package uses the same name for a function. 
...
In this notebook, we load the `dplyr` package despite that risk, with the knowledge that we will not be using the base R versions of these particular functions.
